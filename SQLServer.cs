...
{
    /// <summary>
    /// sqlserver数据库操作类(sql语句中的参数前缀是 '@'(不带单引号))
    /// </summary>
    public class SQLServer
    {
        ...
        /// <summary>
        /// 泛型重载 二
        /// <para>执行查询,返回实体对象数组集合.无值和错误返回null</para>
        /// <para>注意*:实体属性名称和数据库查询语句引用的别名必须一样,大小写不限.</para>
        /// <para>参数T:返回结果集的元素实体对象类型</para>
        /// <para>参数Q:包含有查询参数值的实体对象</para>
        /// <para>参数1:查询语句</para>
        /// <para>参数2:含有查询参数值实体对象的实例(该实例属性名与参数名须一致)</para>
        /// </summary>
        ...
        /// <para>参数1:查询语句</para>
        /// 注意:(1.参数占位标记的顺序和参数数组元素顺序保持一致2.至少写两个参数,如果只有一个参数,那么再加一个参数,否则将会判定到泛型重载)
        /// </summary>
        /// <param name="sql">查询语句</param>
        ...
        /// <summary>
        /// 泛型重载:
        /// <para>执行一个标量查询,返回一个值,无值或发生异常都返回null(参数来自于实体对象)</para>
        /// <para>参数Q:参数实体对象类型</para>
        /// <para>参数1:查询语句</para>
        /// 参数2:含有查询参数值实体对象的实例(该实例属性名与参数名须一致)
        /// </summary>
        ...
        /// <summary>
        /// 泛型重载:
        /// <para>执行一个非查询,返回受影响的行数,发生异常返回-999,(参数来自于实体对象)</para>
        /// <para>参数Q:参数实体对象类型</para>
        /// <para>参数1:查询语句</para>
        /// 参数2:含有查询参数值实体对象的实例(该实例属性名与参数名须一致)
        /// </summary>
        ...
        /// <returns></returns>
        public int ExecuteProcedure(string proc, string outparaname, System.Data.SqlDbType outparatype, out object outparavalue)
        {
            this.InItCmdProc(proc, null, new Dictionary<string, System.Data.SqlDbType>() { { outparaname, outparatype } });
            // 执行过程
        ...
        /// <returns></returns>
        public int ExecuteProcedure(string proc, string paraname, object paravalue,
            string outparaname, System.Data.SqlDbType outparatype, out object outparavalue)
        {
            this.InItCmdProc(proc, new Dictionary<string, object>() { { paraname, paravalue } },
                new Dictionary<string, System.Data.SqlDbType>() { { outparaname, outparatype } });
            // 执行过程
            int reint = this.Procedure(null);
            // 输出参数赋值
            outparavalue = this.cmd.Parameters[outparaname].Value;
        ...
        public int ExecuteProcedure(string proc, Dictionary<string, object> paradict,
            string outparaname, System.Data.SqlDbType outparatype, out object outparavalue)
        {
            this.InItCmdProc(proc, paradict, new Dictionary<string, System.Data.SqlDbType>() { { outparaname, outparatype } });
            // 执行过程
            int reint = this.Procedure(null);
            // 输出参数赋值
            outparavalue = this.cmd.Parameters[outparaname].Value;
        ...
        /// <returns></returns>
        public int ExecuteProcedure(string proc, Dictionary<string, SqlDbType> outparadict,
        ...
        /// <returns></returns>
        public int ExecuteProcedure(string proc, string paraname, object paravalue,
            Dictionary<string, SqlDbType> outparadict, out Dictionary<string, object> outparavaluedict)
        {
            // 初始化命令
        ...
        * ******************************************************************************************************/
        ...
        private void InItCmdProc(string proc, Dictionary<string, object> parasdict,
            Dictionary<string, System.Data.SqlDbType> outparasdict)
        {
            this.cmd = new SqlCommand(proc);
            ...
            if (parasdict != null && parasdict.Keys.Count > 0)
            {
                ...
                {
                    this.cmd.Parameters.Add(new SqlParameter(key, parasdict[key]));
                }
            }
            // 加入输出参数
            if (outparasdict != null && outparasdict.Keys.Count > 0)
            {
                ...
                for (int i = 0; i < outparasdict.Keys.Count; i++)
                {
                    ...
                    SqlParameter outpara = new SqlParameter(key, outparasdict[key]);
        ...
        private void GetPropValueAddParameters<Q>(Q instance, string currpara)
        {
            ...
            if (field != null)
            {
                ...
                this.cmd.Parameters.Add(new SqlParameter(currpara, paravalue));
                return;
            }
        ...
        /// <summary>
        /// 执行插入语句,并且返回自增长ID,如果发生错误,返回-999.
        /// 查询参数是0个或至少2个
        /// 注意:1.参数占位标记的顺序和参数数组元素顺序保持一致
        ///     2.至少写两个参数,如果只有一个参数,那么再加一个参数,否则将会判定到泛型重载
        /// </summary>
        ...
        /// <param name="paravalue"></param>
        /// <returns></returns>
        public long InsertAutoID(string sqlhalf, params object[] paravalue)
        {
            string sql = this.SqlAutoCmptInsert(sqlhalf, 1);
            this.InItCmd(sql, paravalue);
            object newid = this.SelectScalar();
            return newid == null ? -999 : long.Parse(newid.ToString());
        }
        ...
        /// <summary>
        /// 重载 执行插入语句,并且返回自增长ID,如果发生错误,返回-999
        /// </summary>
        /// <param name="sqlhalf"></param>
        /// <param name="instance"></param>
        /// <returns></returns>
        public long InsertAutoID<Q>(string sqlhalf, Q instance)
        {
            string sql = this.SqlAutoCmptInsert(sqlhalf, 1);
            this.InItCmd<Q>(sql, instance);
            object newid = this.SelectScalar();
        ...
        /// <param name="autoid">是否需要拼接上获取自增长ID的语句(默认=0不需要 1=需要)</param>
        /// <returns></returns>
        private string SqlAutoCmptInsert(string insertSql, int autoid = 0)
        {
            ...
            sqlpart.Append(")");
            if (autoid == 1)
                sqlpart.Append(";SELECT SCOPE_IDENTITY()");
